...
/* ------------------------------------------------------------------ */
/*  Styles (inlined to avoid separate CSS build step)                  */
/* ------------------------------------------------------------------ */
...
function buildToolbarConfig(): ToolbarGroup[] {
  return [
    {
      buttons: [
        {
...
const EMOJI_CATEGORIES: { icon: string; name: string; emojis: EmojiEntry[] }[] = [
  {
...
function createEmojiPicker(editor: Editor): HTMLElement {
...
function main(): void {
  ...
  /**
   * Flag to suppress echoing content back to the extension when we
   * receive an update from it (avoids infinite loops).
   */
  ...
  // Insert toolbar before the scroll container, both inside #editor
  ...
  window.addEventListener('message', (event: MessageEvent) => {
    ...
    switch (msg.type) {
      ...
      case 'update': {
        if (typeof msg.content === 'string') {
          // Preserve cursor position by saving and restoring the selection
          const { from, to } = editor.state.selection;
          ignoreNextUpdate = true;
          editor.commands.setContent(msg.content);

          // Clamp the previous selection to the new document length
